...
class TestPageConfigSvgIntegration:
    ...
    def test_default_svg_uses_a1_dimensions(self):
        """PAGE-12: SVG with no page/width/height defaults to A1 (1684 × 2384)."""
        sch = _simple_schematic()
        svg = sch.get_svg_string()
        assert 'width="1684.0"' in svg
        assert 'height="2384.0"' in svg

    def test_default_svg_viewbox_matches_a1(self):
        """PAGE-13: viewBox present and width/height match A1 (viewBox is fit-to-content)."""
        sch = _simple_schematic()
        svg = sch.get_svg_string()
        # width/height on <svg> still reflect A1 page dimensions
        assert 'width="1684.0"' in svg
        assert 'height="2384.0"' in svg
    ...
    def test_export_svg_with_a3_page(self, tmp_path):
        """PAGE-14: export_svg with PageConfig.a3() sets A3 dimensions."""
        sch = _simple_schematic()
        out = tmp_path / "a3.svg"
        sch.export_svg(str(out), page=PageConfig.a3())
        content = out.read_text()
        assert 'width="842.0"' in content
        assert 'height="1191.0"' in content

    def test_get_svg_string_with_a4_page(self):
        """PAGE-15: get_svg_string(page=a4()) respects A4 dimensions."""
        sch = _simple_schematic()
        svg = sch.get_svg_string(page=PageConfig.a4())
        assert 'width="595.0"' in svg
        assert 'height="842.0"' in svg

    def test_render_svg_with_page(self, tmp_path):
        """PAGE-16: render(fmt='svg', page=...) respects PageConfig."""
        sch = _simple_schematic()
        out = tmp_path / "render.svg"
        sch.render(str(out), fmt="svg", page=PageConfig.a2())
        content = out.read_text()
        assert 'width="1191.0"' in content
        assert 'height="1684.0"' in content
    ...
    def test_custom_page_dimensions_in_svg(self):
        """Custom PageConfig dimensions appear in SVG output as width/height."""
        sch = _simple_schematic()
        cfg = PageConfig(width=1200.0, height=900.0)
        svg = sch.get_svg_string(page=cfg)
        assert 'width="1200.0"' in svg
        assert 'height="900.0"' in svg
        # viewBox is fit-to-content — verify it is present but not necessarily full-page
        assert 'viewBox="' in svg

    def test_a0_landscape_svg_dimensions(self):
        """A0 landscape page produces correct wide canvas in SVG."""
        sch = _simple_schematic()
        cfg = PageConfig.a0(landscape=True)
        svg = sch.get_svg_string(page=cfg)
        # Landscape A0: 3370 wide × 2384 tall
        assert 'width="3370.0"' in svg
        assert 'height="2384.0"' in svg
